...
?>
<section x-data="window.bitflan.components.blocklist_component()" x-init="init(<?php echo alpinify($js_errors) ?>, <?php echo $this->options->get('homepage-tlds') ?>)" @keyup.enter="submit()">
    ...
    <template x-if="others_render || suggestions_render" x-transition>
        ...
        <div class="result-content-inner">
            <div class="result-main-title clearfix">
				<div class="left-title-area plus">
                    <span>
                        <svg xmlns="http://www.w3.org/2000/svg" width="17.812" height="17.812" viewBox="0 0 54.194 54.194">
                    ...
                    </span><?php echo lang('blocklist_name') ?></div>
			</div>
            <div class="row">
                <div class="col-lg-6">
                    <div class="result-content-inner">
                        <div class="extension-area">
                            <ul id="tlds_result_column1">
                                <template x-for="url in others_render">
                                    <li x-data="{ data: { status: 'blank', link: '#' } }" x-init="data = await singleQuery(url)">
                                        <a target="_blank" x-cloak x-show="data.status == 'clear'" class="right-by-wo-btn green-by-btn<?php echo(!$this->options->get('enable-buy-button') ? " d-none" : ""); ?>"><span>Clear</span></a>
                                        <a target="_blank" x-cloak x-show="data.status == 'blocked'" class="right-by-wo-btn red-by-btn<?php echo(!$this->options->get('enable-whois-button') ? " d-none" : ""); ?>"><span>Blocked</span></a>
                                        ...
                                        <div :class="data.status == 'blocked' && 'text-danger'" id="keyword_com" class="left-extension" x-text="url"></div>
                ...
                <div class="col-lg-6">
                    <div class="result-content-inner">
                        <div class="extension-area">
                            <ul id="tlds_result_column1">
                                <template x-for="url in suggestions_render">
                                    <li x-data="{ data: { status: 'blank', link: '#' } }" x-init="data = await singleQuery(url)">
                                        <a target="_blank" x-cloak x-show="data.status == 'clear'" class="right-by-wo-btn green-by-btn<?php echo (!$this->options->get('enable-buy-button') ? " d-none" : ""); ?>"><span>Clear</span></a>
                                        <a target="_blank" x-cloak x-show="data.status == 'blocked'" class="right-by-wo-btn red-by-btn<?php echo(!$this->options->get('enable-whois-button') ? " d-none" : ""); ?>"><span>Blocked</span></a>
                                        <a target="_blank" x-cloak x-show="data.status == 'blank'" class="right-by-wo-btn grey-by-btn"><span><img src="<?php echo $this->theme->url('assets/images/domainer_loader.svg') ?>" alt="Loader"/></span></a>
                                        <div id="keyword_com" class="left-extension" x-text="url"></div>
                                        <div class="clearfix"></div>
                                    </li>
                                </template>
                            </ul>
...
<section>
<?php
$this->theme->view('components/blocklist_features');
$this->load->view('core/middle_ad_spot');
$this->theme->view('components/blocklist_faq');
